...
front end: navigate to the front end folder through the terminal ('cd front-end' command) and then you will enter the 'npm install' command which will install the packages used in this repo to your local environment, you can see a list of all these packages in the package.json file in the front-end folder as well as the scripts that can be ran against the repo.

back end: same as before navigate to the back end folder ('cd back-end' command). The back end is written in python and so the process for grabbing all the packages is a little different. When operating within this repo you will want to be in the virtual environment for it (venv) which gives your local environment access to the resources and packages that the back end requires. Activate the venv with the 'source ./venv/bin/activate' command. To confirm that you have entered the venv check that your command line intereface looks like this: '(venv) ➜  back-end git:(main)'
